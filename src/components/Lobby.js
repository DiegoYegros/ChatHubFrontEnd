...
const Lobby = ({ joinRoom, rooms, connection, connectionError, onRetryConnection }) => {
    ...
    return (
        <div 
            className={`card position-fixed top-0 end-0 mt-3 me-3 p-3 connection-status rounded ${isConnected ? 'bg-success' : (connectionError ? 'bg-danger card-hoverable-error' : 'bg-secondary')}`} 
            style={{ 
                width: '100px', 
                display: 'flex', 
        ...
        >
            ...
            : connectionError ?
                <img width="24" height="24" src="https://img.icons8.com/ios/50/error--v1.png" alt="error--v1"/>
                : 
    ...
    return (
        <Container style={{ height: 'calc(100vh - 40px)', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <ConnectionStatusBox isConnected={!!connection} onErrorClick={connectionError ? onRetryConnection : null} />
